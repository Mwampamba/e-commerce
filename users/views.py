...
class EmailValidation(View):
    def post(self, request):
        ...
        if not validate_email(email):
            return JsonResponse({'email_error': 'You have entered an invalid email address!'}, status=400)

        if Account.objects.filter(email=email).exists():
            return JsonResponse({'email_error': ' This email is already taken, try another one'}, status=409)
...
class Registration(View):
    ...
    def post(self, request):
            ...
            if password == "":
                messages.error(
                    request, 'You have to fill in all required fields')
                return render(request, 'authentication/registration.html', context)

            if len(password) < 8:
                messages.error(
                    request, 'Password length must be greater than 8 characters.')
                return render(request, 'authentication/registration.html', context)
...
def activate_customer_account(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        user.save()
        messages.success(
            request, 'Your account successfully activated!')
...
def password_validation(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request, 'You can now create a new password')
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                user.save()
                # auth.logout(request) loggout after changing password
